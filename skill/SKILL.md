...
The `$HOME/.oci_starter_profile` file is a Bash script, not a dotenv file. OCI Starter sources it after the project `terraform.tfvars`, so an `export TF_VAR_<name>=...` value is a user-level override for the matching Terraform input. It commonly contains direct exports and conditional exports. For example:

```bash
if [ "$TF_VAR_compartment_ocid" = "__TO_FILL__" ] || [ -z "$TF_VAR_compartment_ocid" ]; then
  export TF_VAR_compartment_ocid="ocid1.compartment.oc1..."
fi
export TF_VAR_public_ip_filters='["203.0.113.10/32"]'
```

This example uses placeholder values only. Never source or execute the profile to inspect settings, and never print its values. OCI Starter tooling may source it when it runs.
...
6. Find the generated application directory (expected `src/app`; if absent, locate it from the README) and implement or copy the customer program there. Adapt only the files required for the customer application, such as its dependencies, configuration, tests, and health endpoints, following the scaffold's conventions.
   Before editing `src/app`, identify the user instruction that authorizes each intended application change. If none exists, do not edit it.
7. Keep secrets out of source control. Use OCI variables, Vault/secret references, or documented local environment variables as appropriate. When a generated `terraform.tfvars` value is literally `__TO_FILL__`, use the profile-inspection rules above before treating it as undefined. If the profile has a direct or conditional export for either `setting` or `TF_VAR_setting`, report it as profile-backed without reading aloud, copying, or logging its value; let `starter.sh` source the profile rather than copying the secret into source control. For an unset `compartment_ocid` or `public_ip_filters`, first recommend that the user store it as `TF_VAR_compartment_ocid` or `TF_VAR_public_ip_filters` in `$HOME/.oci_starter_profile` instead of `terraform.tfvars`; `public_ip_filters` must be a Terraform list, for example `['203.0.113.10/32']`. If they explicitly prefer project-local configuration, allow them to set it in `terraform.tfvars`. If the profile has neither export and the user does not choose project-local configuration, ask the user to configure the value before deployment.
