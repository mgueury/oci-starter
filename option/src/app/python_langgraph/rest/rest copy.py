...
def accepts_event_stream(accept: str | None) -> bool:
    """Return whether OCI requested a streaming response for this invocation."""
...
async def stream_run_events(
    ...
    if auth_user is None:
        raise RuntimeError("An authenticated user is required to run the agent")
...
async def wait_for_run(
    ...
    async with thread.lock:
        try:
            # Same run semantics as the streaming endpoint, but return final state.
            input_messages = [*thread.messages, *new_messages]
            final_messages = input_messages
...
def sse_response(
    ...
    return StreamingResponse(
        stream_run_events(thread_id, payload, auth_user),
...
async def stream_run(
    ...
    if accepts_event_stream(accept):
        return sse_response(thread_id, payload, auth_user)
    return await wait_for_run(thread_id, payload, auth_user)
