...
if [ "$DEPLOY_WITH_DOCKER" == "true" ]; then
    ...
    # Resource Manager keeps its existing platform selection in docker_build.
    if [ "$TF_VAR_infra_as_code" != "from_resource_manager" ] || [ "$TF_VAR_deploy_type" == "hosted_app" ]; then
        HOST_ARCHITECTURE=$(uname -m)
        if [[ " $HOST_ARCHITECTURES " == *" $HOST_ARCHITECTURE "* ]]; then
            export DOCKER_DEFAULT_PLATFORM=$DOCKER_TARGET_PLATFORM
        elif docker buildx inspect --bootstrap 2>/dev/null | grep -Eq "(^|[[:space:],])${DOCKER_TARGET_PLATFORM}([,[:space:]]|$)"; then
        ...
        elif [ "$(type -t docker)" = "alias" ] && podman build --help 2>/dev/null | grep -q -- '--platform'; then
            # Podman exposes cross-platform builds directly rather than through Buildx.
            export DOCKER_DEFAULT_PLATFORM=$DOCKER_TARGET_PLATFORM
            auto_echo "Podman cross-platform build - Host Architecture $HOST_ARCHITECTURE - DOCKER_DEFAULT_PLATFORM $DOCKER_TARGET_PLATFORM"
        else
